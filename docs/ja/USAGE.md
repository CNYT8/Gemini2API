...
| ログ | ログレベル、保持期間 |
| ウェブセッションクリーンアップ | 有効化トグル、保持時間（hours）、クリーンアップ間隔（hours）、ピン留めスキップ（デフォルト: true）。API 呼び出しにより Gemini ウェブ側に蓄積されたセッション記録を定期的にクリーンアップします。ピン留めされたセッションは保持され、保持期間はプロキシのコンテキスト期間よりはるかに長く設定されているためアクティブな複数ターン会話には影響しません |
