...
전체 목록은 Sub2API의 Gemini API/CLI 모델 카탈로그와 동일합니다. 일반 용도 기본 모델은 `gemini-2.0-flash`, 고품질 용도는 `gemini-2.5-pro`, 이미지 생성은 `*-image` 모델을 사용합니다. 기존 `gemini-pro`, `gemini-flash`, `gemini-flash-thinking` 별칭도 계속 지원됩니다.
