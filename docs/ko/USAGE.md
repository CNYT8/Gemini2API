...
**성능**:
- 회전 전략: round-robin (순차) / least-used (최소 사용)
- 계정당 최대 동시 요청: 1-10
...
| 모델 ID | 설명 |
|--------|------|
| `gemini-2.5-pro` | 최신 Pro 모델 (고성능) |
| `gemini-2.5-flash` | 최신 Flash 모델 (빠른 응답) |
| `gemini-2.5-flash-thinking` | Flash 사고 모드 (심화 분석) |
| `gemini-2.0-flash` | 이전 Flash 모델 |
| `gemini-2.0-flash-thinking` | 이전 Flash 사고 모드 |
